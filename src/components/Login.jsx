...
export default function Login() {
  ...
  async function entrar(e) {
    ...
    setErro(null);
    setCarregando(true);
    const email = usuario.includes("@") ? usuario : `${usuario}@admin.com`;
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <form onSubmit={entrar} style={{ width: "100%", maxWidth: 360, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-2)", margin: "0 0 22px" }}>Entre com seu usuário e senha</p>

        <label style={{ fontSize: 12, color: "var(--text-2)", display: "block", marginBottom: 5 }}>Usuário</label>
        <input type="text" value={usuario} onChange={(e) => setUsuario(e.target.value)} required autoFocus style={inputStyle} />
