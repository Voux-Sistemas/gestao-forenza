  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
        <button onClick={() => setAberto(true)} style={btnPrimary}><Plus size={16} /> Novo cliente</button>
      </div>
          ...
          {lista.map((c) => (
            <div key={c.id} style={linha}>
              ...
              <span style={{ width: 110, textAlign: "right" }}>
                <button onClick={() => alternar(c)} style={btnMini}>{c.ativo ? "Desativar" : "Ativar"}</button>
              </span>
            </div>
          ))}
        </div>
      )}
      {aberto && <FormCliente onFechar={() => setAberto(false)} onOk={() => { setAberto(false); carregar(); }} />}
    </div>
  );
}
...
function Oficinas() {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
        <button onClick={() => setAberto(true)} style={btnPrimary}><Plus size={16} /> Nova oficina</button>
      </div>
        ...
        <div style={tabela}>
          ...
          {lista.map((o) => (
            <div key={o.id} style={linha}>
              ...
              <span style={{ width: 110, textAlign: "right" }}>
                <button onClick={() => alternar(o)} style={btnMini}>{o.ativo ? "Desativar" : "Ativar"}</button>
              </span>
            </div>
          ))}
        </div>
      )}
      {aberto && <FormOficina onFechar={() => setAberto(false)} onOk={() => { setAberto(false); carregar(); }} />}
    </div>
  );
}

function FormCliente({ onFechar, onOk }) {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    const { error } = await supabase.from("clientes").insert({ nome: nome.trim(), contato: contato.trim() || null });
    setSalvando(false);
  ...
  return (
    <Overlay onFechar={onFechar}>
      <h3 style={{ fontSize: 16, fontWeight: 600, margin: "0 0 16px" }}>Novo cliente</h3>
      <label style={lbl}>Nome</label>
      <input value={nome} onChange={(e) => setNome(e.target.value)} autoFocus style={inp} />
      <label style={{ ...lbl, marginTop: 14 }}>Contato (opcional)</label>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
        ...
        <button onClick={salvar} disabled={salvando} style={{ ...btnPrimary, flex: 1 }}>{salvando ? "Salvando…" : "Cadastrar"}</button>
      </div>
    </Overlay>
  );
}

function FormOficina({ onFechar, onOk }) {
  const [campos, setCampos] = useState({ nome_empresa: "", cidade: "", telefone: "", documento: "", maquinario: "", qtd_pessoas: "" });
  const [erro, setErro] = useState(null);
  const [salvando, setSalvando] = useState(false);
  ...
  return (
    <Overlay onFechar={onFechar}>
      <h3 style={{ fontSize: 16, fontWeight: 600, margin: "0 0 16px" }}>Nova oficina</h3>
      ...
      <label style={{ ...lbl, marginTop: 14 }}>Maquinário (opcional)</label>
