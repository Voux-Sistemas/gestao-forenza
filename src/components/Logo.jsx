...
export function MarcaForenza({ size = 30, mono }) {
  // `mono` (ex.: "#fff") força a cor do anel — útil sobre fundos escuros.
  return (
    <svg width={size} height={size} viewBox="0 0 44 44" fill="none" aria-hidden="true" style={{ display: "block", color: mono || "var(--text)", flexShrink: 0 }}>
      <circle cx="22" cy="22" r="19" stroke="currentColor" strokeWidth="4.5" />
      <circle cx="22" cy="22" r="9.5" fill={mono ? "#6fd08a" : "var(--brand)"} />
    </svg>
  );
}
